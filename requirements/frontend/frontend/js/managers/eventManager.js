...
export class EventManager
{
	...
	async handleInputFiles(target)
	{
		...
		const response = await apiService.settings.updateImage({avatar :  formData, reset_image : false})
	...
	async handleDeleteOfImage(target)
	{
		...
		//for now
		const response = await apiService.settings.updateImage({avatar :  null, reset_image : true})
    ...
    async handleAddOfPassword(target)
    {
		...
		await apiService.settings.updatePassword({new_password : new_password, current_password :  current_password})
	}
	async handleNewUsername(target)
	{
        ...
        if (!inputValue || inputValue.includes(' '))
			...
			await apiService.settings.updateUsername({new_username : inputValue})
